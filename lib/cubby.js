...
(function () {
	...
	Cubby.prototype = {
		get: function (key) {
			if (key in this._db) {
				return _.isObject(this._db[key]) ? this._clone(this._db[key]) : this._db[key];
			} else {
				return null;
			}
		},
		...
		set: function (key, value) {
			if (arguments.length === 1) {
				...
				for (key in values) this._db[key] = _.isObject(values[key]) ? this._clone(values[key]) : values[key];
			} else {
				this._db[key] = _.isObject(value) ? this._clone(value) : value;
			}
			this._save();
		},
			...
			path.forEach(function (segment, key) {
				if (!_.isObject(current) && key !== path.length-1) {
					...
					if (key === path.length-1) {
						current[segment] = _.isObject(value) ? this._clone(value) : value;
					} else {
		...
		getPath: function (path) {
			...
			return _.isObject(current) ? this._clone(current) : current;
		},
